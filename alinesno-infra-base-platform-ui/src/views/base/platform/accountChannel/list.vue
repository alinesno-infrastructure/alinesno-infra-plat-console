<template>
   <div class="app-container">
      <el-row :gutter="20">
         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="AccountChannelList" @selection-change="handleSelectionChange">
               ...
               <!-- 业务字段-->
               <el-table-column label="头像" align="center" key="roleAvatar" prop="roleAvatar" v-if="columns[0].visible" />
               <el-table-column label="名称" align="center" key="roleName" prop="roleName" v-if="columns[1].visible" :show-overflow-tooltip="true" />
               <el-table-column label="描述" align="center" key="responsibilities" prop="responsibilities" v-if="columns[1].visible" :show-overflow-tooltip="true" />
               <el-table-column label="链接" align="center" key="typeOwner" prop="typeOwner">
                  <template #default="scope">
                     <el-button type="success" bg text>
                           <i class="fa-solid fa-link"></i>&nbsp;打开
                     </el-button>
                  </template>
               </el-table-column>
                  ...
                  <template #default="scope">
                     <el-tooltip content="修改" placement="top" v-if="scope.row.id !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['platform:AccountChannel:edit']"></el-button>
                     </el-tooltip>
                     <el-tooltip content="删除" placement="top" v-if="scope.row.id !== 1">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['platform:AccountChannel:remove']"></el-button>
                     </el-tooltip>
                  </template>
      ...
      <el-dialog :title="title" v-model="open" width="900px" append-to-body>
         <el-form :model="form" :rules="rules" ref="AccountChannelRef" label-width="100px">
            <el-row>
               <el-col :span="24">
...
import {
   ...
   addAccountChannel
} from "@/api/base/platform/accountChannel";
...
function handleDelete(row) {
   ...
   proxy.$modal.confirm('是否确认删除勾选的数据项？').then(function () {
      return delAccountChannel(tmpIds);
   }).then(() => {
      getList();
...
function handleUpdate(row) {
   reset();
   const AccountChannelId = row.id || ids.value;
   getAccountChannel(AccountChannelId).then(response => {
