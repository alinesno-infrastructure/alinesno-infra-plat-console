      ...
      <el-row :gutter="20">
         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="PartnerList" @selection-change="handleSelectionChange">
               ...
               <!-- 业务字段-->
               <el-table-column label="应用名称" align="center" key="PartnerName" prop="PartnerName" v-if="columns[0].visible" />
               ...
               <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="修改" placement="top" v-if="scope.row.PartnerId !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:Partner:edit']"></el-button>
                     </el-tooltip>
                     <el-tooltip content="删除" placement="top" v-if="scope.row.PartnerId !== 1">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['system:Partner:remove']"></el-button>
                     </el-tooltip>
                  </template>
      ...
      <el-dialog :title="title" v-model="open" width="900px" append-to-body>
         <el-form :model="form" :rules="rules" ref="databaseRef" label-width="100px">
            <el-row>
               ...
               <el-col :span="24">
                  <el-form-item label="应用名称" prop="PartnerName">
                     <el-input v-model="form.PartnerName" placeholder="请输入应用名称" maxlength="50" />
                  </el-form-item>
               </el-col>
            </el-row>
            <el-row>
...
import {
   ...
   updatePartner,
   addPartner
} from "@/api/base/platform/partner";
...
function getList() {
   loading.value = true;
   listPartner(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
      loading.value = false;
...
function handleDelete(row) {
   ...
   proxy.$modal.confirm('是否确认删除应用编号为"' + PartnerIds + '"的数据项？').then(function () {
...
function handleUpdate(row) {
   reset();
   const PartnerId = row.id || ids.value;
...
function submitForm() {
   proxy.$refs["databaseRef"].validate(valid => {
      if (valid) {
         if (form.value.PartnerId != undefined) {
